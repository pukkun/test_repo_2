...
public class Rope : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        gravity = new Vector3(0, 0, gravityStrength);
        cam = Camera.main;
    ...
    private void ApplyConstraint()
    {
        ...
        for (int i = 0; i < totalNodes - 1; i++)
        {
            ...
            if (float.IsNaN(node1.x))
            {
                Debug.LogError("999");
            }
            var node2 = currentNodePositions[i + 1];
            if (float.IsNaN(node2.x))
            {
                Debug.LogError("999");
            }
            ...
            if (float.IsNaN(currentNodePositions[i].x) || float.IsNaN(currentNodePositions[i + 1].x)) 
            {
                Debug.LogError("999");
            }
    ...
    {
        cam = Camera.main;
        startLock = TestStart.transform.position;
        endLock = TestEnd.transform.position;
        isStartLocked = true;
        isEndLocked = true;
    }
#if UNITY_EDITOR
    [UnityEditor.CustomEditor(typeof(Rope))]
