...
public class Reporter : MonoBehaviour
{
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		GUILayout.Box(showSceneContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
		GUILayout.Space(size.x);//GameEnvironment.Instance.GetBuildVersion()
        GUILayout.Label("Unity Version = " + Application.unityVersion + ". GameVersion = 00000" , nonStyle, GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
	...
	private string check = "left";
	private int countCheck = 0;
	private bool showLogStatus = false;
	[SerializeField] private float timePress = 5;
	bool isMouseDown;
	void Update()
	{
		if (Input.GetMouseButtonUp(0))
		...
		if(scenes != null && sceneIndex != -1 && string.IsNullOrEmpty( scenes[sceneIndex] ))
    ...
    private void SaveLogsToDevice()
    {
        ...
        for (int i = 0; i < logs.Count; i++)
        {
            fileContentsList.Add(logs[i].Time + " - Type: " +logs[i].logType + "\n" + logs[i].condition + "\n" + logs[i].stacktrace);
        }
	...
	private void SendLogsToServer()
	{
		...
		for (int i = 0; i < logs.Count; i++)
		{
			fileContentsList.Add(logs[i].logType + System.Environment.NewLine + logs[i].condition + System.Environment.NewLine + logs[i].stacktrace);
		}
		//File.WriteAllLines(filePath, fileContentsList.ToArray());
		string output = string.Join(System.Environment.NewLine, fileContentsList.ToArray());
		//ISFSObject param = SFSObject.NewInstance();
		//param.PutText("data", output);
		//param.PutUtfString("method", "TraceLog_v1");
		//SFSServiceController.Instance.Request(SFSConstant.BASIC, param, result => {
		//	Debug.LogError("Send complete " + result.ToJson());
		//	string userName = result.GetUtfString("u_name");
